...
import '../backend/modal/chat_massage_model.dart';
...
class MessageSendController extends GetxController{
  ...
  connectSocket({required int receiverId, required int userId}) async {
    ...
    // final int userId = await BaseSharedPreference().getInt(SpKeys().userId) ?? "";
      printWrapped("CHAT With user --, --$userId,--$receiverId");
  ...
  sendMessage(receiverId, String message, String type,String link) async {
    final int userId = await BaseSharedPreference().getInt(SpKeys().userId) ?? "";
    print("NEW_MESSAGE:-userId-->$userId,roomId-->");
    print("NEW_MESSAGE:-reciverId-->$receiverId,roomId-->");
    print("NEW_MESSAGE:-userId-->$message,roomId-->");
    try {
      ...
      //   //   socket?.emitWithAck("CHAT_DETAIL",{roomid, userId,receiverId}, ack: (data) {
