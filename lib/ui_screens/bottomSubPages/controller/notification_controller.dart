...
class NotificationController extends GetxController {
  ...
  getNotificationData({String ? refreshType})async {
    ...
    await BaseAPI().get(url: ApiEndPoints().getNotifications, queryParameters: param,showLoader: false)
        .then((value) {
      ...
      if (value?.statusCode == 200) {
        ...
        if(response.status??false){
          if(refreshType == 'refresh'){
            ...
            Get.find<DashBoardController>().getNotificationCount();
  ...
  deleteNotification(id) async {
    ...
    await BaseAPI()
        .post(url: ApiEndPoints().deleteNotifications, data: params)
        .then((value) {
      if (value?.statusCode == 200) {
        int index =
...
///readNotification----
 ...
 /// markNotification----
  markNotification(status) async {
    Map<String, dynamic> params = {'status': status,"notification_id":0};
    await BaseAPI()
        .post(url: ApiEndPoints().markNotifications, data: params)
        .then((value) {
      if (value?.statusCode == 200) {
