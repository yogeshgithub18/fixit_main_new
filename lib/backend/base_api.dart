  ...
  BaseAPI._internal() {
    _dio = Dio(
      BaseOptions(
        ...
        //baseUrl: "https://v1.checkprojectstatus.com/fixit/api/",
  ...
  Future<Response?> post({required String url, dynamic data,bool? showLoader}) async {
    if (await checkInternetConnection()) {
      try {
        ...
        print("language---$languageCode");
        final String token = await BaseSharedPreference().getString(SpKeys().apiToken)??"";
        final response = await _dio.post(url, data: data, options: Options(headers:{"Authorization": "Bearer $token","Accept-Language":languageCode}));
  ...
  Future<Response?> patch({required String url, dynamic data,bool? concatUserId}) async {
    if (await checkInternetConnection()) {
      try {
        ...
        final response = await _dio.patch(url+((concatUserId??false) ? userId : ""), data: data, options: Options(headers:{"Authorization": "Bearer $token","Accept-Language":languageCode}));
  ...
  Future<Response?> put({required String url, dynamic data}) async {
    if (await checkInternetConnection()) {
      try {
        ...
        print("language---$languageCode");
        final String token = await BaseSharedPreference().getString(SpKeys().apiToken)??"";
        final response = await _dio.put(url, data: data, options: Options(headers:{"Authorization": "Bearer $token","Accept-Language":languageCode}));
        BaseOverlays().dismissOverlay();
        return response;
      } on DioError catch (e) {
        BaseOverlays().dismissOverlay();
  ...
  Future<Response?> delete({required String url,dynamic data}) async {
    ...
    if (await checkInternetConnection()) {
      try {
        ...
        languageCode = await BaseSharedPreference().getString('language')??"";
        print("language---$languageCode");
        final response = await _dio.delete(url, data: data, options: Options(headers:{"Authorization": "Bearer $token","Accept-Language":languageCode}));
        BaseOverlays().dismissOverlay();
        return response;
      } on DioError catch (e) {
        BaseOverlays().dismissOverlay();
